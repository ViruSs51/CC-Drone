...
class CameraController:
    __draw_points, __path = [], []
    _run = False
    __start = False
    __started = False
    __command = None

    def __init__(self, get_frame_function: Callable[[], np.ndarray], *args, **kwargs):
    ...
    def running(self):
        ...
        while self.__run:
            ...
            if hands is not None:
                ...
                for i, hand_handedness in enumerate(hands.multi_handedness):
                    ...
                    if label == "Right":
                        ...
                        for key, value in hand.__dict__.items():
                            self.__draw_points.append(
                                hand.__dict__[key]["coord"][:2]
            ...
            for p in self.__draw_points:
                cv2.circle(self.__frame, p[:2], 3, p[2], -1)
...
if __name__ == "__main__":
    ...
    controller = CameraController(get_frame_function=get_frame, capture=cap)
    controller.running()
